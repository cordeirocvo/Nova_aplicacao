  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (res.ok) {
        setShowModal(false);
        fetchReports();
        setReport({ id: "", tipo: "Frio", data: format(new Date(), "yyyy-MM-dd"), responsavel: "", crea: "", profissionalId: "", numero: "", observacoes: "", dadosTecnicos: [] });
      } else {
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-2">
          <Link 
            href="/engenharia/om/profissionais"
            className="bg-slate-100 hover:bg-slate-200 text-slate-700 px-4 py-2 rounded-xl font-bold flex items-center gap-2 transition-all"
          >
          ...
          <div key={r.id} className="bg-white border border-slate-100 rounded-2xl p-5 shadow-sm hover:shadow-md transition-all group">
            ...
            <div className="flex justify-between items-center pt-4 border-t border-slate-50">
              <div className="text-[10px] text-slate-400">
                ...
                {r.crea && <span className="ml-1 text-[9px] bg-slate-100 px-1.5 py-0.5 rounded text-slate-500">CREA: {r.crea}</span>}
              </div>
              <Link 
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4 md:gap-6">
                ...
                <div>
                  ...
                  <select 
                    ...
                    onChange={e => {
                      const p = profissionais.find(item => item.id === e.target.value);
                      if (p) {
                        setReport({...report, profissionalId: p.id, responsavel: p.nome, crea: p.crea});
                      } else {
                        setReport({...report, profissionalId: "", responsavel: "", crea: ""});
                      }
                    }}
                  >
                ...
                {report.crea && (
                  <div className="md:col-span-1">
                    <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Registro CREA</label>
                    <div className="px-4 py-3 rounded-xl bg-slate-100 font-black text-slate-600 text-sm">
                      {report.crea}
                    </div>
                  </div>
                )}
