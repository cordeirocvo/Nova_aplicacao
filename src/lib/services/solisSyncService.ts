...
export class SolisSyncService {
  static async syncAll() {
    fs.appendFileSync(logFile, `\n[${new Date().toISOString()}] Início Solis Sync\n`);

    const usinas = await prisma.usina.findMany({
      where: { apiFornecedor: { equals: 'SOLIS', mode: 'insensitive' } }
    });
    fs.appendFileSync(logFile, `Usinas Solis encontradas: ${usinas.length}\n`);
    ...
    for (const usina of usinas) {
      ...
      try {
        ...
        if (!key || !secret) {
          fs.appendFileSync(logFile, `Sem credenciais para Solis ${usina.nome}\n`);
          continue;
        }
    ...
    fs.appendFileSync(logFile, `[SOLIS-SYNC] Finalizado.\n`);
  }
}
