...
export function TagToggler({ id, prioridade, atividadeExtra, isAdmin }: { id: string, prioridade: boolean, atividadeExtra: boolean, isAdmin: boolean }) {
  ...
  if (!isAdmin) {
    return (
      <div className="flex flex-col mt-1">
        {prioridade && <span className="inline-flex items-center text-[10px] bg-amber-100 text-amber-800 px-1.5 py-0.5 rounded font-bold mb-1"><Star className="w-3 h-3 mr-1 fill-amber-500" /> Prioridade</span>}
  ...
  return (
    <div className="flex flex-col mt-2">
      <label className="flex items-center cursor-pointer hover:bg-black/5 p-1 rounded-md transition-colors w-fit mb-1.5">
        <input 
          type="checkbox" 
          checked={prioridade} 
          disabled={isPending}
      ...
      <label className="flex items-center cursor-pointer hover:bg-black/5 p-1 rounded-md transition-colors w-fit">
        <input 
          type="checkbox" 
